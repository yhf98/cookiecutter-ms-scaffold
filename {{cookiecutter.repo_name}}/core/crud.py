...
from sqlalchemy import func, delete, update, BinaryExpression, ScalarResult, select, false, insert, text
...
class DalBase:
    ...
    def __init__(self, db: AsyncSession = None, model: Any = None, schema: Any = None):
        ...
        self.model = model
        self.schema = schema
        self.queries = self.load_queries('mapper')
    ...
    async def execute_raw_sql(self, raw_sql: str, params: dict = None) -> List[Any]:
        """
        执行原生SQL查询
        :param raw_sql: 原生SQL查询语句
        :param params: 查询参数
        :return: 查询结果
        """
        if params is None:
            params = {}
        result = await self.db.execute(text(raw_sql), params)
    ...
    # async def execute_query(self, query_id: str, params: dict = None) -> List[Any]:
    #     """
    #     执行XML中定义的查询语句
    #     :param query_id: 查询ID
    #     :param params: 查询参数
    ...
    #         raise ValueError(f"Query ID '{query_id}' not found in XML file")
    #     raw_sql = self.queries[query_id]
    #     if params is None:
    #         params = {}
    #     result = await self.db.execute(text(raw_sql), params)
    #     return result.fetchall()
    ...
    async def execute_query(self, module: str, query_id: str, params: dict = None, single_row: bool=True, v_schema=None) -> Any | List[Any]:
        """
        执行XML中定义的查询语句
        :param module: 模块名称
        :param query_id: 查询ID
        ...
        raw_sql = self.queries[module][query_id]
        raw_sql = self.queries[module][query_id]
        params = params or {}
        ...
        if v_schema:
            rows = [v_schema(**row) for row in rows]
    ...
    async def execute_update(self, module: str, query_id: str, params: dict = None) -> None:
        """
        执行XML中定义的更新语句
        :param module: 模块名称
        :param query_id: 查询ID
        ...
        if params is None:
            params = {}
        await self.db.execute(text(raw_sql), params)
