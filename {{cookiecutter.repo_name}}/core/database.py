...
from application.settings import SQLALCHEMY_DATABASE_URL,SQLALCHEMY_DATABASE_URL_SECONDARY, REDIS_DB_ENABLE, MONGO_DB_ENABLE
...
# 创建数据库连接
async_engine_secondary = create_async_engine(
    SQLALCHEMY_DATABASE_URL_SECONDARY,
    echo=False,
    echo_pool=False,
    pool_pre_ping=True,
    pool_recycle=3600,
...
# 创建数据库会话
session_factory_secondary = async_sessionmaker(
    autocommit=False,
    autoflush=False,
    bind=async_engine_secondary,
    expire_on_commit=True,
    class_=AsyncSession
)
...
async def db_getter_secondary() -> AsyncGenerator[AsyncSession, None]:
    """
    获取从数据库会话
    ...
    函数的返回类型被注解为 AsyncGenerator[int, None]，其中 AsyncSession 是生成的值的类型，而 None 表示异步生成器没有终止条件。
    """
    async with session_factory_secondary() as session:
        # 创建一个新的事务，半自动 commit
        async with session.begin():
            yield session
