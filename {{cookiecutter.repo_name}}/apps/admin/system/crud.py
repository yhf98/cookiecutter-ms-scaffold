...
class DictTypeDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.db = db
        self.model = models.AdminDictType
...
class DictDetailsDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.db = db
        self.model = models.AdminDictDetails
...
class SettingsDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.db = db
        self.model = models.AdminSystemSettings
...
class SettingsTabDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        super(SettingsTabDal, self).__init__(db, models.AdminSystemSettingsTab, schemas.SettingsTabSimpleOut)

    async def get_classify_tab_values(self, classify: list[str], hidden: bool | None = False) -> dict:
        """
        获取系统配置分类下的标签信息
        """
        model = models.AdminSystemSettingsTab
    ...
    @classmethod
    def __generate_values(cls, datas: list[models.AdminSystemSettingsTab]) -> dict:
        """
        生成字典值
        """
        return {
...
class TaskDal(MongoManage):
    ...
    def __init__(self, db: AsyncIOMotorDatabase):
        super(TaskDal, self).__init__(db, "admin_system_task", schemas.TaskSimpleOut)
        ...
        pipeline = [
            ...
            },
            {
                '$addFields': {
                    ...
                    'last_run_datetime': {
                        '$ifNull': [
                            {'$arrayElemAt': ['$matched_records.create_at', -1]},
                            None
                        ]
                    }
                }
    ...
    async def get_tasks(
        ...
        last_run_datetime: 在 scheduler_task_record 中获取该任务最近一次执行完成的时间
        """
        v_order_field = v_order_field if v_order_field else 'create_at'
...
class TaskGroupDal(MongoManage):
    ...
    def __init__(self, db: AsyncIOMotorDatabase):
        super(TaskGroupDal, self).__init__(db, "admin_system_task_group")
