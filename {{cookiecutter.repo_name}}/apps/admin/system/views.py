...
from apps.admin.auth.utils.current import AllUserAuth, FullAdminAuth, OpenAuth
from apps.admin.auth.utils.validation.auth import Auth
from .params import DictTypeParams, DictDetailParams, TaskParams
from apps.admin.auth import crud as admin_auth_crud
...
async def sms_send(phone: str, rd: Redis = Depends(redis_getter), auth: Auth = Depends(OpenAuth())):
    user = await admin_auth_crud.UserDal(auth.db).get_data(phone=phone, v_return_none=True)
    if not user:
