...
from apps.admin.system import crud as admin_system_crud
from apps.admin.help import models as admin_help_models
import copy
...
class UserDal(DalBase):
    ...
    async def update_login_info(self, user: models.AdminUser, last_ip: str) -> None:
        """
        更新当前登录信息
        :param user: 用户对象
    ...
    async def create_data(
        ...
        password = data.phone[5:12] if settings.DEFAULT_PASSWORD == "0" else settings.DEFAULT_PASSWORD
    ...
    async def reset_current_password(self, user: models.AdminUser, data: schemas.ResetPwd) -> None:
        """
        重置密码
        :param user:
        :param data:
    ...
    async def update_current_info(self, user: models.AdminUser, data: schemas.UserUpdateBaseInfo) -> Any:
        """
        更新当前用户基本信息
        :param user:
        :param data:
    ...
    async def get_export_headers_options(self, include: list[str] = None) -> dict[str, list]:
        ...
        if include is None or 'gender' in include:
            gender_objs = await admin_system_crud.DictTypeDal(self.db).get_dicts_details(["sys_admin_gender"])
            sys_admin_gender = gender_objs.get("sys_admin_gender", [])
            gender_options = [{"label": i["label"], "value": i["value"]} for i in sys_admin_gender]
    ...
    async def get_import_headers_options(self) -> None:
        ...
        # 性别选择项
        gender_options = await admin_system_crud.DictTypeDal(self.db).get_dicts_details(["sys_admin_gender"])
        gender_item = self.import_headers[3]
        assert isinstance(gender_item, dict)
        sys_admin_gender = gender_options.get("sys_admin_gender")
        gender_item["options"] = [{"label": item["label"], "value": item["value"]} for item in sys_admin_gender]
    ...
    async def update_current_avatar(self, user: models.AdminUser, file: UploadFile) -> str:
        """
        更新当前用户头像
        :param user:
        :param file:
    ...
    async def update_wx_server_openid(self, code: str, user: models.AdminUser, redis: Redis) -> bool:
        """
        更新用户服务端微信平台openid
        :param code:
        :param user:
...
class RoleDal(DalBase):
    ...
    async def delete_datas(self, ids: list[int], v_soft: bool = False, **kwargs) -> None:
        ...
        :param kwargs: 其他更新字段
        """
        user_count = await UserDal(self.db).get_count(v_join=[["roles"]], v_where=[models.AdminRole.id.in_(ids)])
        if user_count > 0:
...
class MenuDal(DalBase):
    ...
    async def get_routers(self, user: models.AdminUser) -> list:
        """
        获取路由表
        ...
        if any([i.is_admin for i in user.roles]):
            ...
            options = [joinedload(models.AdminUser.roles).subqueryload(models.AdminRole.menus)]
    ...
    def generate_router_tree(self, menus: list[models.AdminMenu], nodes: filter, name: str = "") -> list:
        """
        生成路由树
        :param menus: 总菜单列表
        :param nodes: 节点菜单列表
    ...
    def generate_tree_list(self, menus: list[models.AdminMenu], nodes: filter) -> list:
        """
        生成菜单树列表
        :param menus: 总菜单列表
    ...
    def generate_tree_options(self, menus: list[models.AdminMenu], nodes: filter) -> list:
        """
        生成菜单树选择项
        :param menus:总菜单列表
        :param nodes:每层节点菜单列表
...
class DeptDal(DalBase):
    ...
    def generate_tree_list(self, depts: list[models.AdminDept], nodes: filter) -> list:
        """
        生成部门树列表
        :param depts: 总部门列表
    ...
    def generate_tree_options(self, depts: list[models.AdminDept], nodes: filter) -> list:
        """
        生成部门树选择项
        :param depts: 总部门列表
        :param nodes: 每层节点部门列表
...
class TestDal(DalBase):
    ...
    async def test_session_cache1(self):
        ...
        """
        # 第一次查询，并加载用户的所有关联部门项
        sql1 = select(models.AdminUser).where(models.AdminUser.id == 1).options(joinedload(models.AdminUser.depts))
        ...
        sql2 = select(models.AdminUser).where(models.AdminUser.id == 1)
    ...
    async def test_session_cache3(self):
        ...
        sql1 = select(models.AdminUser).options(joinedload(models.AdminUser.depts))
        queryset1 = await self.db.scalars(sql1)
    ...
    async def test_session_cache5(self):
        ...
        """
        # 第一次查询，并加载用户的所有关联部门项
        sql = select(models.AdminUser).where(models.AdminUser.id == 1).options(joinedload(models.AdminUser.depts))
        queryset = await self.db.scalars(sql)
    ...
    async def test_join_form(self):
        ...
        sql = select(models.AdminDept).where(models.AdminDept.is_delete == false())
        sql = sql.join_from(models.AdminUser, models.AdminUser.depts).where(models.AdminUser.id == user_id)
        queryset = await self.db.scalars(sql)
        ...
        # 转换后的 SQL：
        # SELECT
        # 	admin_auth_dept.NAME,
        # 	admin_auth_dept.dept_key,
        # 	admin_auth_dept.disabled,
        # 	admin_auth_dept.order,
        # 	admin_auth_dept.desc,
        # 	admin_auth_dept.OWNER,
        # 	admin_auth_dept.phone,
        # 	admin_auth_dept.email,
        # 	admin_auth_dept.parent_id,
        # 	admin_auth_dept.id,
        # 	admin_auth_dept.create_at,
        # 	admin_auth_dept.update_at,
        # 	admin_auth_dept.delete_datetime,
        # 	admin_auth_dept.is_delete
        # FROM
        # 	admin_auth_user
        # 	JOIN admin_auth_user_depts ON admin_auth_user.id = admin_auth_user_depts.user_id
        # 	JOIN admin_auth_dept ON admin_auth_dept.id = admin_auth_user_depts.dept_id
        # WHERE
        # 	admin_auth_dept.is_delete = FALSE
        # 	AND admin_auth_user.id = 1
    ...
    async def test_left_join(self):
        ...
        """
        # 封装查询语句
        dept_alias = aliased(models.AdminDept)
        v_options = [contains_eager(self.model.depts, alias=dept_alias)]
        v_outer_join = [
            [models.admin_auth_user_depts, self.model.id == models.admin_auth_user_depts.c.user_id],
            [dept_alias, and_(dept_alias.id == models.admin_auth_user_depts.c.dept_id, dept_alias.owner == "张伟")]
        ]
        datas: list[models.AdminUser] = await self.get_datas(
            limit=0,
        ...
        # 原查询语句：
        # DeptAlias = aliased(models.AdminDept)
        # sql = select(self.model).where(self.model.is_delete == false())
        # sql = sql.options(contains_eager(self.model.depts, alias=DeptAlias))
        # sql = sql.outerjoin(models.admin_auth_user_depts, self.model.id == models.admin_auth_user_depts.c.user_id)
        # sql = sql.outerjoin(
        #     DeptAlias,
        #     and_(DeptAlias.id == models.admin_auth_user_depts.c.dept_id, DeptAlias.owner == "张伟")
        # )
        # self.db.expire_all()
        # queryset = await self.db.scalars(sql)
        # result = queryset.unique().all()
    ...
    async def relationship_where_operations_any(self):
        ...
        """
        print("==============================any 方法使用案例1=========================================")
        # 用户表（models.AdminUser）与 部门表（AdminDept）为多对多关系
        # 查找出只有满足关联了部门名称为 "人事一部" 的所有用户，没有关联的则不会查询出来
        sql1 = select(models.AdminUser).where(models.AdminUser.depts.any(models.AdminDept.name == "人事一部"))
        queryset1 = await self.db.scalars(sql1)
        ...
        sql2 = select(models.AdminUser).where(~models.AdminUser.depts.any(models.AdminDept.name == "人事一部"))
        queryset2 = await self.db.scalars(sql2)
        ...
        print("==============================any 方法使用案例3=========================================")
        # 查询出没有关联部门的所有用户
        sql3 = select(models.AdminUser).where(~models.AdminUser.depts.any())
    ...
    async def relationship_where_operations_has(self):
        ...
        """
        print("==============================has 方法使用案例1=========================================")
        # 用户（models.AdminUser）与 帮助问题（models.AdminIssue）为多对一关系
        # 查找出只有满足关联了用户名称为 "ms-api" 的所有帮助问题，没有关联的则不会查询出来
        sql1 = select(admin_help_models.AdminIssue).where(
            admin_help_models.AdminIssue.create_user.has(models.AdminUser.name == "ms-api")
        )
        queryset1 = await self.db.scalars(sql1)
        result1 = queryset1.unique().all()
        for data in result1:
