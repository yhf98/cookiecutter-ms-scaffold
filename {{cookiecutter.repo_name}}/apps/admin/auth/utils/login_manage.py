...
import jwt
from apps.admin.auth import models
...
class LoginManage:
    ...
    @LoginValidation
    async def password_login(self, data: LoginForm, user: models.AdminUser, **kwargs) -> LoginResult:
        """
        验证用户密码
        """
        result = models.AdminUser.verify_password(data.password, user.password)
