...
from apps.admin.auth.models import AdminUser, AdminRole
from core.exception import CustomException
from utils import status
from .validation import AuthValidation
from fastapi import Request, Depends
from application import settings
from core.database import db_getter, db_getter_secondary
...
class OpenAuth(AuthValidation):
    ...
    async def __call__(
        ...
        db_secondary: AsyncSession = Depends(db_getter_secondary),
    ):
        """
        每次调用依赖此类的接口会执行该方法
        """
        if not settings.OAUTH_ENABLE:
            return Auth(db=db, db_secondary=db_secondary)
        try:
            phone = self.validate_token(request, token)
...
class FullAdminAuth(AuthValidation):
    ...
    async def __call__(
        ...
        phone = self.validate_token(request, token)
        options = [
            joinedload(AdminUser.roles).subqueryload(AdminRole.menus),
            joinedload(AdminUser.roles).subqueryload(AdminRole.depts),
            joinedload(AdminUser.depts)
        ]
