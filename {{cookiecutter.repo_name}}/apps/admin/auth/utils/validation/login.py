...
from apps.admin.auth import crud, schemas
...
class LoginForm(BaseModel):
    ...
    normalize_telephone = field_validator('phone')(vali_telephone)
...
class LoginValidation:
    ...
    async def __call__(self, data: LoginForm, db: AsyncSession, request: Request) -> LoginResult:
        ...
        if data.platform not in ["0", "1"] or data.method not in ["0", "1"]:
            ...
            return self.result
        user = await crud.UserDal(db).get_data(phone=data.phone, v_return_none=True)
        if not user:
        ...
        if REDIS_DB_ENABLE:
            count_key = f"{data.phone}_password_auth" if data.method == '0' else f"{data.phone}_sms_auth"
